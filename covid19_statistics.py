...
for idx, pop in enumerate(n_people):
    ratio = ( n_people[idx] * 100 ) / sum_people
...
for idx, pop in enumerate(n_covid):
    ratio = ( n_covid[idx] * 100 ) / sum_covid
    oneM = (n_covid[idx] * (10 ** 6)) / n_people[idx]
    print('| %s | %d | %.1f | %.1f |' % (regions[idx], pop, ratio, oneM))
print()
